...
// Fractionalization Contract ABI - NoKYC version with 3-parameter purchaseShares
export const FRACTIONALIZATION_ABI = [
  ...
  // ✅ CRITICAL: Purchase function with 3 parameters (assetId, shareAmount, maxPricePerShare)
  // This MUST match your deployed contract exactly!
  ...
  // ⚠️ REMOVED: getAssetInvestors - This function doesn't exist in your deployed contract
...
"function calculatePurchaseCost(uint256 _assetId, uint256 _shareAmount) view returns (uint256 totalCost, uint256 platformFeeAmount)",  "function getPendingRequests() view returns (uint256[])",
...
"event FractionsPurchased(uint256 indexed tokenId, address indexed buyer, uint256 amount, uint256 totalCost)",  "event SharesTransferred(uint256 indexed assetId, address indexed from, address indexed to, uint256 amount)",
...
if (import.meta.env.DEV) {
  ...
  // ✅ Log purchaseShares signature for verification
  const purchaseSharesABI = FRACTIONALIZATION_ABI.find(sig => sig.includes('purchaseShares'));
  console.log('   purchaseShares ABI:', purchaseSharesABI);
}
