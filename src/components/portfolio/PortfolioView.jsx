...
const PortfolioView = () => {
  ...
  try {
    ...
    if (!userAssetIds || userAssetIds.length === 0) {
      setUserAssets([]);
      setLoading(false);
      return;
    }
    ...
    const holdingsPromises = userAssetIds.map(async (assetId) => {
      try {
        ...
        // ✅ Check if shares is valid and greater than 0
        if (!shares || shares.isZero()) {
          console.log(`⚠️ No shares for asset ${assetId}`);
          return null;
        }
        ...
        // ✅ Use totalSupply (not totalShares) and pricePerFraction (not pricePerShare)
        const totalSupply = assetDetails.totalSupply;
        const pricePerFraction = assetDetails.pricePerFraction;
        
        // Calculate percentage owned
        const percentageOwned = totalSupply.gt(0) 
          ? shares.mul(10000).div(totalSupply).toNumber() / 100
          : 0;
        
        // Calculate current value
        const currentValue = ethers.utils.formatEther(
          pricePerFraction.mul(shares)
        );
        ...
        return {
          ...
          location: "Dubai, UAE",
          pricePerShare: ethers.utils.formatEther(pricePerFraction),
    ...
    console.log('✅ Portfolio loaded:', holdings);
                          ...
                          <div>
                            <p className="text-sm font-normal text-white">{asset.assetName}</p>
                              <p className="text-xs font-light text-neutral-500">
                                {parseInt(asset.shares).toLocaleString()} {asset.assetType?.includes('weighted') ? 'units' : 'shares'} ({asset.percentageOwned.toFixed(4)}%)
                                </p>
                          </div>
                        </div>
