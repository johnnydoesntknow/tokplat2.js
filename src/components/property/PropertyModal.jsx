...
const PropertyModal = ({ property, onClose }) => {
  ...
  const [percentageInput, setPercentageInput] = useState('0.1');
  const [activeInput, setActiveInput] = useState('percentage'); // 'percentage' or 'units'
  ...
  // Sync unitInput with calculated shareAmount when not actively editing units
  useEffect(() => {
    if (activeInput !== 'units') {
      setUnitInput(shareAmount.toString());
    }
  }, [shareAmount, activeInput]);
  ...
  const handlePercentageSelect = (value) => {
    ...
    const units = Math.floor((value / 100) * parseInt(property.totalShares));
    setUnitInput(units.toString());
  };
  ...
  const handlePercentageInputChange = (e) => {
    ...
    if (!isNaN(num) && num > 0) {
      const clamped = Math.min(maxPercentage, Math.max(0.001, num));
      setPercentage(clamped);
      const units = Math.floor((clamped / 100) * parseInt(property.totalShares));
      setUnitInput(units.toString());
    }
  };
  ...
  const handleUnitInputChange = (e) => {
    ...
    if (!isNaN(units) && units > 0) {
      const pct = (units / parseInt(property.totalShares)) * 100;
      const clamped = Math.min(maxPercentage, Math.max(0.001, pct));
      setPercentage(clamped);
      setPercentageInput(clamped.toFixed(4));
    }
  };
              ...
              {!hasReachedLimit ? (
                <div className="bg-neutral-950 border border-neutral-800 rounded-sm p-6">
                  ...
                  <div className="space-y-6">
                    ...
                    {/* ✅ NEW: Dual Input - Percentage & Units */}
                    <div className="space-y-4">
                      <label className="text-xs font-light uppercase tracking-wider text-neutral-400 block">
                        Custom Amount
                      </label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {/* Percentage Input */}
                        <div>
                          <label className="block text-xs font-light text-neutral-400 mb-2">
                            Ownership %
                          </label>
                          <div className="relative">
                            <input
                              type="number"
                              value={activeInput === 'percentage' ? percentageInput : percentage.toFixed(4)}
                              onChange={handlePercentageInputChange}
                              onFocus={() => setActiveInput('percentage')}
                              step="0.001"
                              min="0.001"
                              max={maxPercentage}
                              placeholder="5.0"
                              disabled={loading || fetchingShares}
                              className="w-full bg-black border border-neutral-800 px-4 py-3 text-white pr-8 focus:border-neutral-600 focus:outline-none transition-colors disabled:opacity-50"
                            />
                        ...
                        <div>
                          <label className="block text-xs font-light text-neutral-400 mb-2">
                            {property.shareType === 'weighted' ? 'Units' : 'Shares'}
                          </label>
                          <input
                            type="number"
                            value={activeInput === 'units' ? unitInput : shareAmount}
                            ...
                            placeholder="50000"
                            disabled={loading || fetchingShares}
                            className="w-full bg-black border border-neutral-800 px-4 py-3 text-white focus:border-neutral-600 focus:outline-none transition-colors disabled:opacity-50"
                          />
                        </div>
                      </div>
                      ...
                      <p className="text-xs text-neutral-500 text-center">
                        {fetchingShares ? (
                          ...
                          'Switch between percentage or exact units - both sync automatically'
                        )}
                      </p>
                    </div>
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span>Acquire {formatPercentage(percentage)}%</span>
                      </>
                    )}
                  </button>
                )}
